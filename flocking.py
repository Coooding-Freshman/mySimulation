    ...
    def calculate_control(self, group, target, local):
        ...
        for agent in group:
            if not (self is agent):
                dis = np.sqrt((self.x - agent.record[self.i][0])**2 + (
                    self.y - agent.record[self.i][1])**2)
                force = phi_alpha(dis)
                gradient_part += force * np.array([
                    agent.record[self.i][0] - self.x, agent.record[self.i][1] -
                    self.y
                ]) / dis
        ...
        for myNeighbor in self.adj:
            if myNeighbor.flag == self.flag:
                #pass
                consensus_part += np.array([
                    myNeighbor.record_vel[self.i][0] - self.dx,
                    myNeighbor.record_vel[self.i][1] - self.dy
                ])
            else:
                othersum += myNeighbor.record_vel[self.i]
                num += 1

        if num != 0:
            othersum /= num
            consensus_part += self.sensor.dynamic(
                np.concatenate((np.array([self.dx, self.dy]), othersum)))

        #part III: claculate force that process by feedback from target
        self.sensor.myfilter([member.sensor for member in self.adj], self.i,
                             target, local)
...
if __name__ == "__main__":
    ...
    for i in range(2):
        ...
        temp, = plt.plot(
            time_range,
            track[:, i],
            label='target x\'s velocity trajectory'.format(mp[i]))
        plt.legend()
        plt.title(
            "The agent's {0} dimension velocity in x group".format(mp[i]))
        plt.show()
    ...
    for i in range(2):
        ...
        plt.plot(
            time_range,
            track[:, i + 2],
            label='target y\'s velocity trajectory'.format(mp[i]))
        plt.legend()
        plt.title(
            "The agent's {0} dimension velocity in y group".format(mp[i]))
        plt.show()

    for s in [5, 15, 20]:
        end = s * 100 - 1
        plt.figure("flocking in {0} s".format(s))
        follower1 = plt.scatter(
            [agent.record[end, 0] for agent in group1],
            [agent.record[end, 1] for agent in group1],
            color='b')
        follower2 = plt.scatter(
            [agent.record[end, 0] for agent in group2],
            [agent.record[end, 1] for agent in group2],
            color='g')
        for agent in wholeGroup:
            lens = np.sqrt(agent.record_vel[end, 0]**2 + agent.record_vel[
                end, 1]**2)
            plt.arrow(
                agent.record[end, 0],
                agent.record[end, 1],
                agent.record_vel[end, 0] / lens,
                agent.record_vel[end, 1] / lens,
                head_width=0.5,
                head_length=0.5,
                fc='k',
                ec='k')
        ...
        target1 = plt.scatter(
            local[end, 0], local[end, 1], color='r', marker='^', s=40)
