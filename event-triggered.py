...
yy = xx
xy = np.array(
    [[0.5, 0.5, 0, 0], [0, 0.5, 0.5, 0], [0, 0, 0.5, 0.5], [0.5, 0, 0, 0.5]])
yx = np.transpose(xy)
h = 1
H = h * np.eye(2)
q = 1
...
def updateB(t):
    ...
    D[0, 0] = 0.01  #+np.sin(t/100)/60
    D[1, 1] = 0.01  #-np.cos(t/100)/90
...
class sensor():
    ...
    def __init__(self, flag):
        ...
        self.pij = np.array(
            [40 * np.array(np.eye(2)) for i in range(numOfGroup1)])
        sensor.tag += 1
        self.counter = 0
    ...
    def myfilter(self, group, step, location):
        ...
        if self.flag == 1:
            du = self.dynamic(
                np.concatenate((np.array([self.x, self.y]), othersum)))
        else:
            du = self.dynamic(
    ...
    def _updataPK(self, group):
        ...
        g_ij = 0
        if self.flag == 1:
            add_term = np.dot(A - np.dot(self.k, H), A.transpose()) + np.dot(
                A, (A - np.dot(self.k, H).transpose()))
        if self.flag == 2:
            add_term = np.dot(B - np.dot(self.k, H), B.transpose()) + np.dot(
                B, (B - np.dot(self.k, H).transpose()))
        for agent in group:
            if agent.flag != self.flag and self.flag == 1:
                ...
                if xy[self.tag][agent.tag] != 0:
                    ...
                    self.pij[agent.tag] = np.dot(
                        np.dot(temp1, self.pij[agent.tag]), temp2) + np.dot(
                            np.dot(C, self.pij[agent.tag]), D) + sigma
            if agent.flag != self.flag and self.flag == 2:
                ...
                if yx[self.tag][agent.tag] != 0:
                    ...
                    self.pij[agent.tag] = np.dot(
                        np.dot(temp1, self.pij[agent.tag]), temp2) + np.dot(
                            np.dot(D, self.pij[agent.tag]), C) + sigma
            if agent.flag == self.flag and self.cal_distance(agent):
                g_ij += 0.02
        ansp = np.zeros((2, 2))
        ansk = np.zeros((2, 2))
        ...
        if self.flag == 1:
            ansk = (np.dot(np.dot(A, self.p), H) + np.dot(
                np.dot(C, sumpij), H) + sigma * g_ij * np.dot(A, H)
                    ) * np.linalg.inv(inv)
            temp = A - np.dot(self.k, H)
            ansp = np.dot(np.dot(temp, self.p), temp.transpose()) + np.dot(
                self.k, self.k) * q + np.dot(np.dot(C, sump),
                                             C.transpose()) + g_ij * add_term
        else:
            ansk = (np.dot(np.dot(B, self.p), H) + np.dot(
                np.dot(D, sumpij), H) + sigma * g_ij * np.dot(B, H)
                    ) * np.linalg.inv(inv)
...
if __name__ == "__main__":
    ...
    for i in range(numOfGroup2):
        plt.plot(
            group2[i].record[:1000, 0],
            group2[i].record[:1000, 1],
            linestyle="--")
    ...
    for j in range(step):
        error[j] = sum([
            sum((group1[i].record[j] - location[j, 0:2])**2)
            for i in range(numOfGroup1)
        ]) / numOfGroup1
    plt.plot(range(step), error, label=r"$ x$-group")
    for j in range(step):
        error[j] = sum([
            sum((group2[i].record[j] - location[j, 2:4])**2)
            for i in range(numOfGroup2)
        ]) / numOfGroup2
